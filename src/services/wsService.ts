...
export function initWebSocket(server: Server, path = "/ws") {
  ...
  wss.on("connection", async (ws: WebSocket) => {
    ...
    ws.on("message", async (buf) => {
      ...
      broadcastToChat(chatId, JSON.stringify({
        type: "chat:message",
        chatId,
        authorId: userId,
...
export function getWss() {
  if (!wss) throw new Error("WS not initialized. Call initWebSocket(server) first.");
  return wss;
}
...
function removeFromMapSet(map: Map<string, Set<string>>, key: string, value: string) {
...
async function saveMessage(authorId: string, chatId: string, content: string): Promise<IMessage> {
