...
export async function createChat(data: CreateChatRequest): Promise<IChat> {
  ...
  try {
    ...
    const participantIds: Types.ObjectId[] = data.participantIds.map(id=> ensureObjectId(id));
...
export async function addParticipant(chatIdRaw: string, userIdRaw: string): Promise<IChat> {
...
export async function removeParticipant(chatIdRaw: string, userIdRaw: string) : Promise<IChat> {
  const chatId = ensureObjectId(chatIdRaw);
  const userId = ensureObjectId(userIdRaw);
